


#7345 ish
#12611 ishAC
#76674
#97070

include("graph_generation.jl")
using graph_generation
using LightGraphs
using JLD

max_seed = 1
max_nodes = 1
added = 0

srand(1);


levels = 10  ;   #number of levels in binary tree
new_edges = 0
new_nodes = 10
a_0 = -6.5;
#a_0 = -6; 


#g = BinaryTree(levels);  #generate tree
#g = watts_strogatz(10000,20,0.2)
g = Grid([100,100])
n = nv(g) ; #get initial number of nodes

#b = (rand(n) .< 8 / n)*5. ;  #generate b vector

#### SPARSE NORMAL
b = zeros(n);
nz = 30;
nz_indices = rand(1:n,nz)
for i in nz_indices
    b[i] = randn()*3+5
end


#### DENSE NORMAL
#b = randn(n)


gen_b = copy(b);  # save for later
g = graph_generation.randEdgeGen(g,new_edges);  
A = Array{Int64,1}[];
L =  SparseMatrixCSC{Int64,Int64}[];

added = 0
for i in 1:new_nodes 
    push!(L, laplacian_matrix(g));
    g = graph_generation.addPrefNode(g,b, a_0);
    #connects = zeros(2^levels-2+i)  #-1 for -1 1 coding;
    connects = zeros(n+i-1)  #-1 for -1 1 coding;
    connects[neighbors(g,nv(g))] = 1;
    push!(A,connects);
    println(i)
    added += 1
end



#save("graph_jld/WS_10000-20-02b+0e+100n_normal_dense_seed1.jld","connections",A,"laplacians",L,"b" , gen_b,"a_0",a_0)


gen_a = lufact(L[1])\(gen_b-mean(gen_b));
gen_a -= mean(gen_a);
invLogit(x) = 1./(1.+e.^-x)   ;
gen_p = invLogit(gen_a+a_0)
gen_p[find(b.>0)]
mean(gen_p)
sum(A[1])








data = load("graph_jld/");
L = data["laplacians"];
A = data["connections"];
a_0 = data["a_0"];
gen_b = data["b"];


precisions = []
for k = 1:10:5000
    bb = copy(gen_b);
    total = []
    for i =1:size(A)[1]

        #pred_a = lufact(L[i])\(pred_b-mean(pred_b));
        gen_a = lufact(L[i])\(bb-mean(bb));


        #top_pred = sortperm(pred_a)[length(gen_a)-k:length(gen_a)]
        top_gen = sortperm(gen_a)[length(gen_a)-5000:length(gen_a)]

        true_connects =find(A[i].==1)
        gen_prec = length(intersect(top_gen,true_connects))/length(true_connects)
        println(gen_prec)
        #pred_prec = length(intersect(top_pred,true))/k
        
        append!(bb,0)
        append!(total, gen_prec)

    end
    print(mean(total))
    append!(precisions,mean(total))
end




#plot(1:10:5000,precisions)
#plot(1:10:5000, 1:10:5000./length(A[1]))

    
bb = copy(gen_b);
pb = copy(pred_b)
total_pred = []
total_gen = []
#for i =1:size(A)[1]
for i  = 1:10
    pred_a = lufact(L[i])\(pred_b-mean(pred_b));
    pred_a -= mean(pred_a)
    gen_a = lufact(L[i])\(bb-mean(bb));
    gen_a -= mean(gen_a);


    top_pred = sortperm(pred_a)[length(gen_a)-5000:length(gen_a)]
    top_gen = sortperm(gen_a)[length(gen_a)-5000:length(gen_a)]

    true_connects =find(A[i].==1)
    pred_prec = length(intersect(top_pred,true_connects))/length(true_connects)
    gen_prec = length(intersect(top_gen,true_connects))/length(true_connects)

    println(gen_prec)
    println(pred_prec)
    println("--------")
    append!(bb,0)
    append!(pred_b,0)
    append!(total_pred, pred_prec)
    append!(total_gen, gen_prec)
end
println("##########")
println(mean(total_gen))
println(mean(total_prec))
